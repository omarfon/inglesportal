...
wp.customize.controlConstructor['kirki-column'] = wp.customize.Control.extend({
	...
	ready: function () {
		...
		this.container.on('change', 'input', function () {
			...
			var colClass = 'column column-' + Math.round(12 / column);

			for (i = 1; i <= column; i++) {
				html += '<div class="' + colClass + '"><div class="inner">' + i + '</div></div>';
			}

			jQuery(this).closest('label').find('.columns').html(html);

			jQuery(this).closest('label').find('.columns .column').colResizable({
				liveDrag:true,
				gripInnerHtml:"<div class='inner'></div>",
