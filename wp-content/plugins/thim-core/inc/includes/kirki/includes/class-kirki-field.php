...
if ( !class_exists( 'Kirki_Field' ) ) {
	...
	class Kirki_Field {
		...
		/**
		 * Used to hide field when it's initialize.
		 *
		 * @var bool
		 * @thim_created
		 */
		...
		protected $raw_active_callback = null;
		...
		public function __construct( $config_id = 'global', $args = array() ) {
			$args['raw_active_callback'] = !empty( $args['active_callback'] ) ? $args['active_callback'] : null;
		...
		protected function set_field( $whitelisted_properties = array() ) {
			...
			//$args['raw_active_callback'] = 6999999999;
		...
		protected function set_settings() {
			...
			//$this->settings['active_callback'] = $this->_active_callback;
		}
		...
		protected function set_output() {
			...
			foreach ( $this->output as $output ) {
				...
				if ( !isset( $output['property'] ) && !in_array( $this->type, array(
						'kirki-typography',
						'background'
					), true )
				) {
		...
		protected function set_js_vars() {
			...
			//array_unshift( $this->js_vars, array( 'active_callback' => $this->raw_active_callback ) );
			//$this->js_vars['active_callback'] = $this->raw_active_callback;
			...
			$js_vars = array(
				'active_callback' => $this->raw_active_callback
			);
