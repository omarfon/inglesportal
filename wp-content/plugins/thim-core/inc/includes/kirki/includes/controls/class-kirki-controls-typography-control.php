			...
			<div class="wrapper">
				...
				<# if ( data.default['font-family'] ) { #>
					...
					<div class="font-family">
						<h5>{{ kirkiL10n.global['font-family'] }}</h5>
						<select id="kirki-typography-font-family-{{{ data.id }}}" placeholder="{{ kirkiL10n.global['select-font-family'] }}"></select>
					</div>
					<# if ( true === data.show_variants || false !== data.default.variant ) { #>
						<div class="variant hide-on-standard-fonts kirki-variant-wrapper">
							<h5>{{ kirkiL10n.global['variant'] }}</h5>
					...
					<# if ( true === data.show_subsets ) { #>
						<div class="subsets hide-on-standard-fonts kirki-subsets-wrapper">
							<h5>{{ kirkiL10n.global['subsets'] }}</h5>
				...
				<# if ( data.default['font-size'] ) { #>
					<div class="font-size">
						<h5>{{ kirkiL10n.global['font-size'] }}</h5>
				...
				<# if ( data.default['line-height'] ) { #>
					<div class="line-height">
						<h5>{{ kirkiL10n.global['line-height'] }}</h5>
				...
				<# if ( data.default['letter-spacing'] ) { #>
					<div class="letter-spacing">
						<h5>{{ kirkiL10n.global['letter-spacing'] }}</h5>
				...
				<# if ( data.default['text-align'] ) { #>
					<div class="text-align">
						<h5>{{ kirkiL10n.global['text-align'] }}</h5>
						<input type="radio" value="inherit" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-inherit" <# if ( data.value['text-align'] === 'inherit' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-inherit">
								...
								<span class="screen-reader-text">{{ kirkiL10n.global['inherit'] }}</span>
							</label>
						</input>
						<input type="radio" value="left" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-left" <# if ( data.value['text-align'] === 'left' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-left">
								...
								<span class="screen-reader-text">{{ kirkiL10n.global['left'] }}</span>
							</label>
						</input>
						<input type="radio" value="center" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-center" <# if ( data.value['text-align'] === 'center' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-center">
								...
								<span class="screen-reader-text">{{ kirkiL10n.global['center'] }}</span>
							</label>
						</input>
						<input type="radio" value="right" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-right" <# if ( data.value['text-align'] === 'right' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-right">
								...
								<span class="screen-reader-text">{{ kirkiL10n.global['right'] }}</span>
							</label>
						</input>
						<input type="radio" value="justify" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-justify" <# if ( data.value['text-align'] === 'justify' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-justify">
								...
								<span class="screen-reader-text">{{ kirkiL10n.global['justify'] }}</span>
							</label>
						</input>
					</div>
				<# } #>
				...
				<# if ( data.default['text-transform'] ) { #>
					<div class="text-transform">
						<h5>{{ kirkiL10n.global['text-transform'] }}</h5>
						<select id="kirki-typography-text-transform-{{{ data.id }}}">
							<option value="none"<# if ( 'none' === data.value['text-transform'] ) { #>selected<# } #>>{{ kirkiL10n.global['none'] }}</option>
							<option value="capitalize"<# if ( 'capitalize' === data.value['text-transform'] ) { #>selected<# } #>>{{ kirkiL10n.global['capitalize'] }}</option>
							<option value="uppercase"<# if ( 'uppercase' === data.value['text-transform'] ) { #>selected<# } #>>{{ kirkiL10n.global['uppercase'] }}</option>
							<option value="lowercase"<# if ( 'lowercase' === data.value['text-transform'] ) { #>selected<# } #>>{{ kirkiL10n.global['lowercase'] }}</option>
							<option value="initial"<# if ( 'initial' === data.value['text-transform'] ) { #>selected<# } #>>{{ kirkiL10n.global['initial'] }}</option>
							<option value="inherit"<# if ( 'inherit' === data.value['text-transform'] ) { #>selected<# } #>>{{ kirkiL10n.global['inherit'] }}</option>
						</select>
					</div>
				<# } #>
