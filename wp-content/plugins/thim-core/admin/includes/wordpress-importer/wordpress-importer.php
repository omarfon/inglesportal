...
if ( class_exists( 'WP_Importer' ) ) {
	class WP_Import extends WP_Importer {
		...
		/**
		 * Set import media file.
		 *
		 * @since        0.3.1
		 *
		 * @param $enable
		 *
		 * @thim_created 26/08/2016
		 */
		public function set_import_media( $enable = false ) {
			$this->fetch_attachments = $enable;
		}
		...
		function import_options() {
			...
			<form action="<?php echo admin_url( 'admin.php?import=wordpress&amp;step=2' ); ?>" method="post">
				...
				<p class="submit">
					<input type="submit" class="button" value="<?php esc_attr_e( 'Submit', 'wordpress-importer' ); ?>" />
				</p>
			</form>
			<?php
		}
		...
		function process_menu_item( $item ) {
			...
			if ( 'taxonomy' == $_menu_item_type && isset( $this->processed_terms[intval( $_menu_item_object_id )] ) ) {
				...
				/**
				 * Fix issue that missing menu items
				 */
				if ( isset( $this->processed_posts[intval( $_menu_item_object_id )] ) ) {
					$_menu_item_object_id = $this->processed_posts[intval( $_menu_item_object_id )];
				}
		...
		function fetch_remote_file( $url, $post ) {
			...
			if ( !$headers ) {
				...
				$error = new WP_Error( 'import_file_error', __( 'Remote server did not respond', 'wordpress-importer' ) );
				$error->add( 'import_file_error', sprintf( '%s, %s', $url, $upload['file'] ) );
				thim_add_log( sprintf( "Import media error %s", $url ) );
			...
			if ( $headers['response'] != '200' ) {
				...
				$error = new WP_Error( 'import_file_error', sprintf( __( 'Remote server returned error response %1$d %2$s', 'wordpress-importer' ), esc_html( $headers['response'] ), get_status_header_desc( $headers['response'] ) ) );
				$error->add( 'import_file_error', sprintf( '%s, %s', $url, $upload['file'] ) );
		 ...
		 * @since 0.4.0
		 *
		 * @param $attachment_demo_image_id
		 */
		protected function __process_posts( $attachment_demo_image_id ) {
			...
			foreach ( $this->posts as $post ) {
				...
				if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
					...
					if ( 'attachment' == $postdata['post_type'] ) {
						...
						$comment_post_ID = $post_id = $this->_remap_url_images( $remote_url, $attachment_demo_image_id );
					} else {
						$comment_post_ID = $post_id = wp_insert_post( $postdata, true );
						do_action( 'wp_import_insert_post', $post_id, $original_post_ID, $postdata, $post );
					}
