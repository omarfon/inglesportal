...
class Thim_Getting_Started extends Thim_Admin_Sub_Page {
		...
		if ( ! $this->is_first_time() ) {
			Thim_Notification::add_notification(
				array(
					'id'          => 'gs-visited',
					'type'        => 'warning',
					'content'     => __( "It looks like you already have setup the theme :)", 'thim-core' ),
					'dismissible' => true,
					'global'      => false,
				)
			);
	...
	public function main_template() {
		update_option( 'thim_getting_started_first_time', false );

		parent::main_template();
	}

	/**
	 * Is first time visit getting started page.
	 *
	 * @since 0.8.9
	 *
	 * @return bool
	...
	public function is_first_time() {
		$option = get_option( 'thim_getting_started_first_time', true );
