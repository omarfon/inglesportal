...
class Thim_Menu_Manager extends Thim_Singleton {
	...
	public static function get_link_iframe_how_to_use() {
		return apply_filters( 'thim_core_megamenu_video_how_to_use', 'https://www.youtube.com/embed/2tM1LFFxeKg' );
	}
	 ...
	 * @since 0.9.1
	 */
	private function notification_support() {
		global $wp_version;
	...
	public static function render_template( $template, $args = array() ) {
		$dir_path = THIM_CORE_ADMIN_PATH . '/views/mega-menu/';
	...
	private function init_hooks() {
		...
		add_filter( 'thim_walker_nav_menu_edit_start_el', array( $this, 'add_fields_menu' ), 10, 4 );
	 ...
	 */
	public function update_nav_menu( $nav_menu_selected_id ) {
	 ...
	 * @return string
	 */
	public function add_fields_menu( $item, $depth, $args, $id ) {
		$menu_id  = $item->ID;
		$settings = $this->get_settings( $menu_id );

		ob_start();
		self::render_template( 'extra-fields.php', array(
			'$menu_id'  => $menu_id,
			'$settings' => $settings,
		) );
	...
	public function enqueue_scripts( $page ) {
		...
		wp_enqueue_style( 'thim-font-awesome', THIM_CORE_URI . '/admin/assets/css/font-awesome.min.css', array(), '4.7.0' );
