			...
			if ( 'nav_menu_item' == $post['post_type'] ) {
				...
				if ( is_numeric( $nav_menu_id ) ) {
					...
					if ( !empty( $post['postmeta'] ) ) {
						$post['postmeta'] = apply_filters( 'tc_importer_meta_menu_item', $post['postmeta'], $post );
...
//			var_dump( $has_posts );
	...
	function add_log( $file ) {
		...
		file_put_contents( ABSPATH . '/' . $file, ob_get_clean() );
	}
	...
	public function clean_demo_content() {
		...
		$t             = microtime( true );
		$this->_clean_posts();
		$this->add_log( 'clean-posts.txt', $wpdb->queries, microtime( true ) - $t );


		$wpdb->queries = array();
		$t             = microtime( true );
		$this->_clean_taxes();
		$this->add_log( 'clean-taxes.txt', $wpdb->queries, microtime( true ) - $t );


		$wpdb->queries = array();
		$t             = microtime( true );
		$this->_clean_comments();
		$this->add_log( 'clean-comment.txt', $wpdb->queries, microtime( true ) - $t );


		$wpdb->queries = array();
		$t             = microtime( true );
		$this->_clean_users();
		$this->add_log( 'clean-users.txt', $wpdb->queries, microtime( true ) - $t );
	...
	private function _clean_taxes() {
		...
		if ( $terms = $wpdb->get_results( $query ) ) {
			...
			// Reset object terms and count
			/*$tt_format = array_fill( 0, sizeof( $tt_ids ), '%d' );
			$query     = $wpdb->prepare( "
				SELECT tt.term_id, tr.term_taxonomy_id, tt.taxonomy, GROUP_CONCAT(object_id) as object_ids
				FROM {$wpdb->term_relationships} tr
				INNER JOIN {$wpdb->term_taxonomy} tt ON tt.term_taxonomy_id = tr.term_taxonomy_id
				WHERE tt.term_taxonomy_id IN(" . join( ',', $tt_format ) . ")
				GROUP BY tt.term_taxonomy_id
			", $tt_ids );

			if ( $results = $wpdb->get_results( $query ) ) {
				foreach ( $results as $result ) {
					$object_ids = explode( ',', $result->object_ids );
					foreach ( $object_ids as $object_id ) {
						$terms = wp_get_object_terms( $object_id, $result->taxonomy, array( 'fields' => 'ids', 'orderby' => 'none' ) );
						if ( 1 == count( $terms ) && isset( $default ) ) {
							$terms = array( $default );
						} else {
							$terms = array_diff( $terms, array( $result->term_id ) );
							if ( isset( $default ) && isset( $force_default ) && $force_default )
								$terms = array_merge( $terms, array( $default ) );
						}
						$terms = array_map( 'intval', $terms );
						wp_set_object_terms( $object_id, $terms, $result->taxonomy );
					}
				}
			}*/
