...
if ( ! class_exists( 'Thim_Widget_Importer_Service' ) ) {
	...
	class Thim_Widget_Importer_Service {
		 ...
		 * @return mixed
		 */
		public static function mapping_nav_menu_in_content( $content ) {
			$match = false;
			...
			foreach ( $strings as $index => $string ) {
				$menu_id     = $ids[ $index ];
				$new_menu_id = Thim_Widget_Importer_Service::get_map_nav_menu( $menu_id );
		...
		private static function mapping_layout_builder() {
			...
			foreach ( $widgets as $key => $widget ) {
				...
				$content           = self::mapping_nav_menu_in_content( $content );
