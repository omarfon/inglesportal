...
add_action( 'wp_ajax_nopriv_thim_update_theme_mods', 'thim_import_demo_page_builder' );
...
add_filter( 'thim_envato_link_purchase', function () {
	return 'https://themeforest.net/item/education-wordpress-theme-education-wp/14058034';
} );
...
add_filter( 'thim_core_args_form_subscribe', function ( $defaults ) {
	return array(
...
add_filter( 'thim_style_default_uri', function () {
	return trailingslashit( get_template_directory_uri() ) . 'inc/data/default.css';
} );

add_filter( 'thim_core_field_name_custom_css_theme', function () {
	...
	if ( class_exists( 'Thim_Notification' ) ) {
		Thim_Notification::add_notification(
			array(
				...
				'type'        => 'warning',
				'content'     => __( 'Thim Core plugin is the newest upgrade version of Thim Framework and it substitutes Thim Framework. Please, <strong>do not re-activate Thim Framework.</strong>', 'eduma' ),
				'dismissible' => true,
				'global'      => true,
			)
		);
